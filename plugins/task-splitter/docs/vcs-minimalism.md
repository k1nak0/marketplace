...
This is the same test §2 applies to a design doc's old `## Implementation
...
is not telling it to record local behaviour. This plugin writes no source
code itself, but the check still governs how `implementation-workflow` reads
this shared policy, and it's why §2 above already forbids the equivalent
narrative in a design doc's body.

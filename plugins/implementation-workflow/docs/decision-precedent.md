...
This document mirrors the copies in the `task-splitter` and `light-workflow`
plugins. All three install independently, so each carries its own copy; **if
...
Any point where you're about to treat an implementation choice as settled, or
write an ADR of your own — anywhere `vcs-minimalism.md` §2's half-day test
could turn what you're about to do into an ADR. Run the check *before* you
decide, not after: catching a conflict once the code is already written or the
design doc already edited means undoing that work too.
...
| Nothing on the books touches this | No conflict — decide normally, per `vcs-minimalism.md` §2/§3 |
...
- **`issue-refinement`**, which holds the user relationship directly: raise it
  with `AskUserQuestion` on the spot — show the existing ADR's `Decision` (or
  the rejected alternative) next to the direction you're about to take, and
  continue only once the user explicitly agrees.
- **`implementer`**, which has no interactive access to the user: don't try to
  resolve it yourself. Record it in `why-notes.md` under a `## ADR conflict`
  heading — the existing ADR, the direction you're about to take, and why you
  believe it's still right despite the conflict — and flag it prominently in
  your return value instead of writing past it silently. You may continue the
  implementation using your best judgement while the conflict is open, exactly
...
- **`implementer`**, before writing a draft ADR of its own (Step 3).
